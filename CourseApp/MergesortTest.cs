...
namespace CourseApp
{
    public class MergesortTest
    {
        ...
        public static int[] MergeSort(int[] resultArray)
        {
            ...
            var res = Merge(left, rigth);
            Console.WriteLine($"{mid + 1} {resultArray.Length - mid} {resultArray[0]} {resultArray[resultArray.Length - 1]}");
            return Merge(left, rigth); // index outside of mass range FIXABLE????????????
        }
    }
}
